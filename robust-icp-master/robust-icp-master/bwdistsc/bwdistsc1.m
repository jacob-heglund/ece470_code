...
% BWDISTSC1 computes Euclidean distance transform of the binary 3D image 
...
% of BW. BW may be 3D array or a cell array of 2D slices. ASPECT is
% 3-component vector defining aspect ratio in the dataset BW. If ASPECT 
% is not specified, isotropic aspect ratio [1 1 1] is used. If MAXVAL
% is specified, the distance transform will be only calculated up to  
% that value; this may  accelerate calculations for nonconvex situations 
% where distance transform is wanted only out to a certain smaller value.
%
% BWDISTSC uses the same algorithm with BWDISTSC. 
%
% BWDISTSC tries to use MATLAB bwdist for 2D scans if possible, which is 
% faster. Otherwise BWDISTSC uses own algorithm to perform 2D scans. Also
% incorporates fix for Matlab version detection by Tudor Dima.
%
%    BY Yuriy Mishchenko  JUL 2011

% This code is free for use or modifications, just please give credit where
% appropriate. And if you modify code or fix bugs, please drop me a message
...
% establish (once) whether will use regionprops (on versions newer than 7.3)

UseRegionProps = exist('regionprops', 'file') && VersionNewerThan(7.3);

% need this to drop pixels from consideration if current distance exceeded
...
% enable using regionprops if available, on m. versions 7.3 and later
% regionprops is actually faster than this code
% version control for using regionprops now outside
...
%    older          |  any       |  false    
% 
% 26.12.2011 - new, Tudor for Yuryi > bwdistc1
